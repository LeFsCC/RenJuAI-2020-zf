...
void NetGameWindow::sendMessage(MessageType type,QString serverAddress)
{
    ...
    switch(type)
    {
        case Message:
            ...
            {
                QMessageBox::warning(0,"warning",QStringLiteral(""),QMessageBox::Ok);
                return;
            }
...
void NetGameWindow::participantLeft(QString userName, QString localHostName, QString time)
{
        ...
        ui->messageBrowser->append(QStringLiteral("%1%2").arg(userName).arg(time));
}
    ...
    while(udpSocket->hasPendingDatagrams())
    {
        ...
        switch(messageType)
        {
            ...
            case Giveup:
                in>>userName>>localHostName;
                if(myColor)
                  QMessageBox::about(this,QStringLiteral(""),QStringLiteral(""));
                else
                  QMessageBox::about(this,QStringLiteral(""),QStringLiteral(""));
                mouseflag=0;
                break;
            case Back:
                ...
                {
                    returnint=QMessageBox::question(this,QStringLiteral(""),QStringLiteral(""),QMessageBox::Yes,QMessageBox::No);
            ...
            case Ready:
                ...
                ui->messageBrowser->setTextColor(Qt::black);
                ui->messageBrowser->setCurrentFont(QFont("Times New Roman",10));
                ui->messageBrowser->append(QStringLiteral("%1").arg(userName));
                if(myReady!=0&&otherReady!=0)
                {
                    ...
                    ui->messageBrowser->append(QStringLiteral(""));
                }
                break;
            case Gamesite:
...
QString NetGameWindow::getUserName()
{
    ...
    foreach (QString string, envVariables) {
        //indexOf????????list???,QRegExp`ˇ?????????????`
        int index = environment.indexOf(QRegExp(string));
        if (index != -1) {
            //stringList???ˇenvironment.at(index)??Ж'='??????
            QStringList stringList = environment.at(index).split('=');
            if (stringList.size() == 2) {
                return stringList.at(1);//at(0)?τ?"USERNAME."ìat(1)????
                break;
            }
        }
    }
...
void NetGameWindow::paintEvent(QPaintEvent *)
{
    ...
    for(int i=0;i<15;i++)
        {
            painter.drawLine(40+i*40,60,40+i*40,620);//?П
            painter.drawLine(40,60+i*40,600,60+i*40);//?П
        }
    ...
    if(mouseflag%2)
    {
        ...
        if(event->x()>=20&&event->x()<=620&&event->y()>=40&&event->y()<=640)
        {
            if(game.judge(x,y))
            {
                ...
                QMessageBox::information(this,QStringLiteral(""),QStringLiteral(""),QMessageBox::Ok);
        }
        else
            QMessageBox::information(this,QStringLiteral(""),QStringLiteral(""),QMessageBox::Ok);
        update();
...
{
    int ret=QMessageBox::question(this,QStringLiteral(""),QStringLiteral(""),QMessageBox::Yes,QMessageBox::No);
...
void NetGameWindow::on_actionScreenshot_triggered()
{
    ...
    QString filename = QFileDialog::getSaveFileName(this,QStringLiteral(""),"", "Image File(*.jpg)");
